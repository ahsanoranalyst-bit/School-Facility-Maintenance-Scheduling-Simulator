...
def generate_multi_report(df, r_type, stats):
    ...
    # Corrected Date Format (DD-MM-YYYY)
    ...
    for _, row in df.iterrows():
        ...
        if r_type == "ORDER":
            ...
            # Formatted date in PDF
            next_date = pd.to_datetime(row['Next_Service']).strftime('%d-%m-%Y')
            pdf.cell(35, 8, next_date, 1, 1, 'C')
...
if 'assets' not in st.session_state:
    st.session_state.assets = pd.DataFrame([
        {"Asset": "Air Conditioners", "Qty": 20, "Avg Age (Yrs)": 3, "Last Service": "2023-10-01", "Warranty": "2025-01-01", "KM": 0},
        {"Asset": "Computers", "Qty": 50, "Avg Age (Yrs)": 2, "Last Service": "2024-01-15", "Warranty": "2025-06-01", "KM": 0},
        {"Asset": "Plumbing System", "Qty": 1, "Avg Age (Yrs)": 15, "Last Service": "2022-05-20", "Warranty": "Expired", "KM": 0}
    ])
...
with tabs[0]:
    ...
    with st.expander("➕ Add New Asset"):
        with st.form("entry_form"):
            ...
            a_date = c4.date_input("Last Service")
            a_war = c5.text_input("Warranty Status")
            a_km = c6.number_input("KM Reading (If applicable)", min_value=0)
            
            if st.form_submit_button("Submit to Ledger"):
                new_data = {"Asset": a_name, "Qty": a_qty, "Avg Age (Yrs)": a_age, "Last Service": str(a_date), "Warranty": a_war, "KM": a_km}
...
with tabs[1]:
    ...
    # Timing change based on distance/age logic
    ...
    # Visual Date Formatting (DD-MM-YYYY)
    display_sched = sched_df[['Asset', 'Last Service', 'Next_Service', 'KM']].copy()
    display_sched['Last Service'] = pd.to_datetime(display_sched['Last Service']).dt.strftime('%d-%m-%Y')
    display_sched['Next_Service'] = pd.to_datetime(display_sched['Next_Service']).dt.strftime('%d-%m-%Y')
