...
def generate_multi_report(df, r_type, stats):
    ...
    for _, row in df.iterrows():
        ...
        if r_type == "ORDER":
            ...
            # Formatting next service date for PDF
...
with tabs[0]:
    ...
    with st.expander("➕ Add New Asset"):
        with st.form("entry_form"):
            ...
            # Date Input
            a_date = c4.date_input("Last Service Date")
            a_war = c5.date_input("Warranty Expiry Date")
            a_km = c6.number_input("KM Reading", min_value=0)
            
            if st.form_submit_button("Save Asset"):
                # Save as string in DD-MM-YYYY for consistent display
                new_data = {
                    "Asset": a_name, 
                    "Qty": a_qty, 
                    "Avg Age (Yrs)": a_age, 
                    "Last Service": a_date.strftime('%d-%m-%Y'), 
...
with tabs[1]:
    ...
    # Handle both datetime and string dates for calculation
    ...
    # Format display table to DD-MM-YYYY
...
with tabs[2]:
    ...
    st.divider()
    c1, c2, c3 = st.columns(3)
    c1.download_button("📥 Admin Audit (Blue)", generate_multi_report(risk_df, "ADMIN", {}), "Admin_Audit.pdf")
    c2.download_button("📥 Summary Report", generate_multi_report(risk_df, "SUMMARY", {}), "Summary.pdf")
    c3.download_button("📋 Vendor Order", generate_multi_report(risk_df, "ORDER", {}), "Order.pdf")
