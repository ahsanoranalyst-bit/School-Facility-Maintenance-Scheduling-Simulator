...
class FacilityPDF(FPDF):
    def header(self):
        ...
        self.cell(0, 5, "Facility Management & Audit System", ln=True, align='C')
        self.ln(20)
...
def generate_pdf_report(df, report_type):
    ...
    # Title logic
    titles = {
        "ADMIN": "EXECUTIVE AUDIT REPORT",
        "VENDOR": "VENDOR PURCHASE ORDER",
        "SUMMARY": "INSTITUTIONAL ASSET SUMMARY"
    }
    
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(0, 10, titles.get(report_type, "REPORT"), ln=True, align='L')
    ...
    if report_type == "VENDOR":
        cols = [("Asset Item", 80), ("Qty", 30), ("Urgency", 40), ("Est. Cost", 40)]
    else:
        cols = [("Asset", 60), ("Qty", 20), ("Risk (PKR)", 40), ("Score", 35), ("Next Svc", 35)]
    ...
    for _, row in df.iterrows():
        if report_type == "VENDOR":
            ...
            status = "CRITICAL" if row['Predictive Score'] < 40 else "ROUTINE"
...
if 'assets' not in st.session_state:
    st.session_state.assets = pd.DataFrame([
        ...
        {"Asset": "Generators", "Qty": 2, "Avg Age (Yrs)": 6, "Last Service": "15-05-2024", "Warranty": "Expired"}
    ])
...
# --- 6. MAIN CONTENT ---
tabs = st.tabs(["📋 Inventory", "📅 Schedule", "📊 Intelligence & Export"])

with tabs[0]:
    st.subheader("Asset Inventory Management")
    
    # Import Option
    up = st.file_uploader("📂 Import Asset Data (Excel/CSV)", type=["xlsx", "csv"])
    if up:
        if up.name.endswith('csv'): st.session_state.assets = pd.read_csv(up)
        else: st.session_state.assets = pd.read_excel(up)
    ...
    with st.expander("➕ Manual Entry Form"):
        with st.form("manual_entry"):
            ...
            age = c3.number_input("Current Age (Yrs)", min_value=0.0)
            ...
            if st.form_submit_button("Add Asset"):
                new_row = {
                    ...
                    "Warranty": warranty.strftime('%d-%m-%Y')
                }
...
with tabs[1]:
    ...
    # Timing logic based on years [cite: 2026-02-10]
...
with tabs[2]:
    ...
    # Predictive Score [cite: 2026-02-14]
    ...
    m2.metric("Critical Assets", len(risk_df[risk_df['Predictive Score'] < 40]))
    m3.metric("System Health", f"{int(risk_df['Predictive Score'].mean())}%")

    st.divider()
    st.subheader("📑 Multi-Format Report Center")
    colA, colB, colC = st.columns(3)
    
    # Download Buttons for 3 Types of Reports
    colA.download_button("📥 Admin Audit Report", generate_pdf_report(risk_df, "ADMIN"), "Admin_Audit.pdf", use_container_width=True)
    colB.download_button("📥 Vendor Order PDF", generate_pdf_report(risk_df, "VENDOR"), "Vendor_Order.pdf", use_container_width=True)
    colC.download_button("📥 Institutional Summary", generate_pdf_report(risk_df, "SUMMARY"), "Overall_Summary.pdf", use_container_width=True)

    st.divider()
    st.plotly_chart(px.bar(risk_df, x='Asset', y='Est. Repair Cost', color='Predictive Score', color_continuous_scale='Blues'), use_container_width=True)
